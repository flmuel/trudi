namespace TRuDI.Backend.Utils
{
    ...
    public static class HtmlStringExtensions
    {
        ...
        public static string GetFnnStatusString(this StatusFNN status, int count = 0)
        {
            ...
            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Power_Low))
            {
                statusItems.Add("Zähler: Energieversorgung niedrig");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Temporary_Error))
            {
                statusItems.Add("Zähler: vorübergehender Fehler");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Permanent_Error))
            {
                statusItems.Add("Zähler: dauerhafter Fehler");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Application_Busy))
            {
                statusItems.Add("Zähler: Anwendung beschäftigt");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Application_Error))
            {
                statusItems.Add("Zähler: Anwendungsfehler");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Application_Alarm))
            {
                statusItems.Add("Zähler: ungewöhnlicher Zustand/Alarm");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Magnetic_Manipulation))
            {
                statusItems.Add("Zähler: magnetische Manipulation");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Connection_Interrupted))
            {
                statusItems.Add("Zähler: Verbindung unterbrochen");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Hardware_Or_Software_Error))
            {
                statusItems.Add("Zähler: allgemeiner Hardware- oder Softwarefehler");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Battery_Disconnected_Or_Shutdown))
            {
                statusItems.Add("Zähler: Batterie getrennt oder Abschaltung");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.External_Alarm))
            {
                statusItems.Add("Zähler: externer Alarm");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Low_Battery))
            {
                statusItems.Add("Zähler: Batterie schwach");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Manipulation))
            {
                statusItems.Add("Zähler: Manipulation");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Clock_Synchronization_Error))
            {
                statusItems.Add("Zähler: Uhrensynchronisierungsfehler");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Sensor_Out_Of_Range))
            {
                statusItems.Add("Zähler: Sensor außerhalb des zulässigen Bereichs");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Consumption_Out_Of_Range))
            {
                statusItems.Add("Zähler: Verbrauch außerhalb des zulässigen Bereichs");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Lack_Of_Flow))
            {
                statusItems.Add("Zähler: fehlender Durchfluss ");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Leakage))
            {
                statusItems.Add("Zähler: Leckage");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Credit_Limit_Exceeded))
            {
                statusItems.Add("Zähler: Guthabengrenze überschritten");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Unauthorized_Access_Attempt))
            {
                statusItems.Add("Zähler: nicht autorisierter Zugriffsversuch");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Historical_Mechanical_Manipulation))
            {
                statusItems.Add("Zähler: Historische mechanische Manipulation");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Historical_Magnetic_Manipulation))
            {
                statusItems.Add("Zähler: Historische magnetische Manipulation");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Historical_Connection_Interrupted))
            {
                statusItems.Add("Zähler: Historische Unterbrechung der Verbindung");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Historical_External_Alarm))
            {
                statusItems.Add("Zähler: Historischer externer Alarm");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Historical_Leakage))
            {
                statusItems.Add("Zähler: Historische Leckage");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Historical_Lack_Of_Flow))
            {
                statusItems.Add("Zähler: Historischer fehlender Durchfluss");
            }

            if (status.OmsStatusWord.HasFlag(OmsStatusWord.Historical_Sensor_Out_Of_Range))
            {
                statusItems.Add("Zähler: Historischer Sensor außerhalb des zulässigen Bereichs");
            }
